...
extension AIService {
    final class SemesterGradesToolDefinition: ToolDefinition, @unchecked Sendable {
        init() {
            super.init(
                functionName: "get_semester_grades",
                displayName: "查询成绩信息",
                category: .query,
                functionDescription: "查询指定学期的成绩信息。调用前会检查当前 jAccount 是否已启用考试与成绩功能；若未启用，则直接返回提示用户前往账户页启用考试与成绩功能。",
        ...
        override func execute(argumentsJSON: String) async -> String {
            do {
                ...
                let result = try await AIService.fetchSemesterGradesResult(
                    account: account,
                    year: arguments.year,
                    semester: semester
                )
                ...
                return AIService.encodeToolExecutionError(
                    .init(error: AIService.examAndGradeToolErrorText(error, subject: "成绩信息"))
                )
            }
        }
